...
//    focusd.common.message.enqueue("exit");     // shutdown the async_main thread if running
      focusd.exit_cleanly();                     // shutdown the daemon
      break;
    case SIGHUP:
      logwrite(function, "caught SIGHUP");
      focusd.configure_focusd();                 // TODO can (/should) this be done while running?
      break;
    case SIGPIPE:
...
void thread_main(Network::TcpSocket sock) {
  while (1) {
    focusd.conn_mutex.lock();
    sock.Accept();
    focusd.conn_mutex.unlock();
    doit(sock);                // call function to do the work
    sock.Close();
  }
  return;
...
void async_main(Network::UdpSocket sock) {
  std::string function = "Focus::async_main";
  int retval;
  ...
  if (retval < 0) {
    ...
    focusd.exit_cleanly();                                  // do not continue on error
  }
...
/***
  while (1) {
    std::string message = focusd.common.message.dequeue();  // get the latest message from the queue (blocks)
...
void doit(Network::TcpSocket sock) {
  std::string function = "Focus::doit";
  char  buf[BUFSIZE+1];
  long  ret;
  std::stringstream message;
  ...
  while (connection_open) {
    ...
    try {
      ...
      sock.id = ++focusd.cmd_num;
      if ( focusd.cmd_num == INT_MAX ) focusd.cmd_num = 0;
...
//                  focusd.common.message.enqueue("exit");     // shutdown the async message thread if running
                    focusd.exit_cleanly();                     // shutdown the daemon
    }
