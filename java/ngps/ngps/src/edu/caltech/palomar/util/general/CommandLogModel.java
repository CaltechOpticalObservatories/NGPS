...
/=============================================================================================*/
public class CommandLogModel extends Object{
  private transient Vector actionListeners;
  private Style newCommandStyle;
  private Style newResponseStyle;
...
/*================================================================================================
/        checkDocumentLength() method
/=================================================================================================*/
  public synchronized void checkDocumentLength(){
         int myLength = MessageDocument.getDefaultRootElement().getDocument().getLength();
        if(myLength > maximumLength){
          try{
//           FTPMessageDocument.remove(myLength - removeLength,removeLength);
           MessageDocument.getDefaultRootElement().getDocument().remove(myLength - removeLength,removeLength);
//           System.out.print("FTPCommandLogModel - Trimming the document" + "\n");
          }catch(javax.swing.text.BadLocationException bl){
            System.out.print("CommandLogModel - Error inserting Message Line 90");
          }
      }// End of the If check
  }
...
/=============================================================================================*/
  ...
  private java.lang.String newFTPMessage = new java.lang.String();
  private CommandLogModel parent;
  private int             default_message_type = COMMAND;
  private int             current_message_type;

  public RunTrimDocument(CommandLogModel parent,java.lang.String nextFTPMessage){
    this.parent = parent;
    current_message_type = default_message_type;
    newFTPMessage = nextFTPMessage;
  }
    public RunTrimDocument(CommandLogModel parent,java.lang.String nextFTPMessage,int message_type){
    this.parent = parent;
...
//      checkFTPDocumentLength();
     int myLength = MessageDocument.getLength();
     if(myLength > maximumLength){
      MessageDocument.remove(myLength - removeLength,removeLength);
     }
     if(current_message_type == COMMAND){
       MessageDocument.insertString(MessageDocument.getLength(),(newFTPMessage + terminator), getDocument().getStyle("command"));
     }
     if(current_message_type == RESPONSE){
       MessageDocument.insertString(MessageDocument.getLength(),(newFTPMessage + terminator), getDocument().getStyle("response"));
     }
     if(current_message_type == ERROR){
      MessageDocument.insertString(MessageDocument.getLength(),(newFTPMessage + terminator), getDocument().getStyle("error"));
     }
      parent.fireActionPerformed(new java.awt.event.ActionEvent(parent,UPDATE_SCROLLBAR_POSITION,"Update ScrollBar Position"));
    }catch(javax.swing.text.BadLocationException bl){
        System.out.print("CommandLogModel - Error inserting Message Line 78");
     }
   }
  public void start(){
