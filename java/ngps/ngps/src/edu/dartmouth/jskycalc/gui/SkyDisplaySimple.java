...
/     class SkyDisplay
/=================================================================================================*/
 public class SkyDisplaySimple extends JComponent implements   MouseMotionListener, KeyListener,  MouseListener{
       ...
       Font        mediumfont   = new Font("Dialog",Font.PLAIN, 15);
       Font        largefont    = new Font("Dialog",Font.PLAIN, 18);
       ...
       Color       telescopeCenterColor  = new Color(225,225,0);  // for object box on display
       Color       telescopeCrossColor   = new Color(228,0,0);  // for object box on display
...
//     presenterKey   this is a variable used extensively in astronomical object selection
  ...
  public   JSkyCalcModel    model;
  public   BrightStar []    bs;
  private  Graphics         g;
  public   AstroObjectsModel myAstroObjectsModel;
  public   JSkyCalcModel     myJSkyCalcModel;
  public   boolean           AIRMASSPLOT       = true;
  private boolean isDisplayPlanets     = true;
  private boolean isDisplayHAGrid      = true;
  private boolean isDisplayAllSky      = true;
  private boolean isDisplayAirmassGrid = true;
  private boolean isDisplayTargets     = true;
  private boolean isDisplaySun         = true;
  private boolean isDisplayMoon        = true;
  private boolean isDisplayBrightStars = true;
  public AllSkyImageProcessor myAllSkyImageProcessor;
  public TelescopesIniReader  myTelescopesIniReader;
  private Color[][]           overlay;
/*================================================================================================
/     SkyDisplaySimple(int xpixIn, int ypixIn,JFrame frame,JSkyCalcModel model)
/=================================================================================================*/
   public SkyDisplaySimple() {
      isInitialized = false;
      this.setDisplayAllSky(false);
      this.setDisplayBrightStars(true);
   }
/*=============================================================================================
/    setOverlay(Color[][] new_overlay)
/=============================================================================================*/
public synchronized void setOverlay(Color[][] new_overlay){
    overlay = new_overlay;
}
/*================================================================================================
...
//     REQUIRED FOR OBJECT PLOTTING AND SELECTION
               SelObjByPos(markedC);
//               synchOutput();
            }
  ...
  void SelObjByPos(Celest incel) {
    /* get input from the graphical display, or wherever, and get the nearest
       ...
       if(presenterKey.size() > 0) {
          ...
          Set      set   = presenterKey.entrySet();
          Iterator iter  = set.iterator();
           while(iter.hasNext()) {
               Map.Entry me = (Map.Entry)iter.next();
               String key = (String)me.getKey();
               objcel = (AstroObject)me.getValue();
          ...
          if(minsep < tolerance) {
              System.out.println("Closest Object = " + currentKey);
              AstroObject currentObject = presenterKey.get(currentKey);
              ...
              myAstroObjectsModel.setSelectedRightAscension(currentObject.Alpha.RoundedRAString(2, ":"));
                ...
                if(AIRMASSPLOT){
                 ...
                 AstroObject co = presenterKey.get(currentKey);
                ...
                this.repaintSkyDisplay();
          }
      }
   }
/*================================================================================================
/       initializeAllSkyImageMonitor()
/=================================================================================================*/
public void initializeAllSkyImageMonitor(){
    myTelescopesIniReader  = new TelescopesIniReader();
    myAllSkyImageProcessor = new AllSkyImageProcessor();
    myAllSkyImageProcessor.setTelescopeIniReader(myTelescopesIniReader);
    myAllSkyImageProcessor.setSkyDisplay(this);
    myAllSkyImageProcessor.startImageMonitor();
}
public AllSkyImageProcessor getAllSkyImageProcessor(){
    return myAllSkyImageProcessor;
}
/*================================================================================================
/       SelBrightByPos(Celest incel)
/=================================================================================================*/
 public void initializeDisplay(int xpixIn, int ypixIn,JFrame frame,JSkyCalcModel model){
   initializeAllSkyImageMonitor();
   this.model = model;
   this.p = model.getPlanets();
   this.o = model.getObsevation();
   this.bs = model.getBrightStars();
   this.SkyWin = frame;
   xpixint     = xpixIn;
   ypixint     = ypixIn;
   xpix        = (double) xpixint;
   ...
   addJSkyCalcModelActionListener();
   tinyhelp = new tinyHelp();
   isInitialized = true;
 }
/*================================================================================================
/         Menu File Exit Action Listener
/=================================================================================================*/
  public void addJSkyCalcModelActionListener(){
...
/=================================================================================================*/
       public void paint(Graphics g) {   // this method does the graphics.
           ...
           // Draw the allsky image onto the image
           if(isDisplayAllSky()){
               for(int js=0;js<myAllSkyImageProcessor.overlay_y-1;js++){
                 for(int ks=0;ks<myAllSkyImageProcessor.overlay_x-1;ks++){
                     g.setColor(overlay[js][ks]);
                    if(overlay[js][ks].getBlue() == 0 & overlay[js][ks].getGreen() == 0 & overlay[js][ks].getRed() == 0){
                       g.setColor(skycolor());
                    }
                    g.fillRect(ks, js, 1, 1);
                }
               }
           }
           ...
           markcoords(3);  // plots the current selected target position on the sky display
           if(isDisplaySun()){
             plotsun();
           }
           if(isDisplayMoon()){
...
/=================================================================================================*/
	   ...
	   if(option == 1) {
              ...
              g2.setPaint(telescopeCrossColor);
              drawcircle(xy[0],xy[1],0.02,"solid",1.5f);
              g2.setPaint(telescopeCenterColor);
              drawline(xy[0]-0.02,xy[1],xy[0]+0.02,xy[1]);
              drawline(xy[0],xy[1]-0.02,xy[0],xy[1]+0.02);

	   }
           if(option == 3){
               double      currenteq = o.w.when.JulianEpoch();
               AstroObject obj       = myAstroObjectsModel.getAstroObject();
               if( obj != null) {
                   Celest cel = obj.precessed(currenteq);
                   xy = SkyProject(lst - cel.Alpha.value, cel.Delta.value, coslat, sinlat);
                   g2.setPaint(Color.RED);
                   drawcircle(xy[0],xy[1],0.01,"solid",1.5f);
...
//                   puttext(xy[0],xy[1],obj.name,smallfont,smallfontmetrics);
              ...
              coslat = Math.cos(o.w.where.lat.radians());
              sinlat = Math.sin(o.w.where.lat.radians());
           }
       }
/*================================================================================================
/     plotsun()
...
/=================================================================================================*/
	  ...
	  double magzpt = 4.7;
          ...
          for(i = 0; i < bs.length; i++) {
             ...
             if(isDisplayAllSky){
                putDot(xy[0],xy[1],magconst1 + magslope * (magzpt - bs[i].m),new Color(0,255,0));
             }
             if(!isDisplayAllSky){
...
//    THIS SECTION DOES THE ACTUAL PLOTTING USING THE presenterKey HASHMAP
//    THIS SECTION IS REMOVED FOR THE "SIMPLE" VERSION OF THE SKY DISPLAY
      Set set = presenterKey.entrySet();
      Iterator iter = set.iterator();
           while(iter.hasNext()) {
               Map.Entry me = (Map.Entry)iter.next();
               String key = (String)me.getKey();
               obj = (AstroObject)me.getValue();
//               System.out.println("Key = " + key + " RA = " + obj.Alpha.value + " Dec = " + obj.Delta.value);
               if( ! obj.name.equals("null")) {
                   cel = obj.precessed(currenteq);
                   xy = SkyProject(lst - cel.Alpha.value, cel.Delta.value, coslat, sinlat);
//                   xy_pixel = xytopix(xy[0],xy[1]);
//                   System.out.println(obj.name+","+xy_pixel[0]+","+xy_pixel[1]);
                   puttext(xy[0],xy[1],obj.name,smallfont,smallfontmetrics);
               }
           }
       }
...
/*=============================================================================================
/     setDisplayPlanets(boolean state)
/=============================================================================================*/
 public void setDisplayPlanets(boolean state){
     isDisplayPlanets = state;
 }
 public boolean isDisplayPlanets(){
     return isDisplayPlanets;
 }
/*=============================================================================================
/     setDisplayHAGrid(boolean state)
/=============================================================================================*/
 public void setDisplayHAGrid(boolean state){
     isDisplayHAGrid = state;
 }
 public boolean isDisplayHAGrid(){
     return isDisplayHAGrid;
 }
 /*=============================================================================================
/     setDisplayHAGrid(boolean state)
/=============================================================================================*/
 public void setDisplayAllSky(boolean state){
     isDisplayAllSky = state;
 }
 public boolean isDisplayAllSky(){
     return isDisplayAllSky;
 }
/*=============================================================================================
/     setDisplayAirmassGrid(boolean state)
/=============================================================================================*/
 public void setDisplayAirmassGrid(boolean state){
     isDisplayAirmassGrid = state;
 }
 public boolean isDisplayAirmassGrid(){
     return isDisplayAirmassGrid;
 }  
/*=============================================================================================
/     setDisplayAirmassGrid(boolean state)
/=============================================================================================*/
 public void setDisplayTargets(boolean state){
     isDisplayTargets = state;
 }
 public boolean isDisplayTargets(){
     return isDisplayTargets;
 }
/*=============================================================================================
/     etDisplaySun(boolean state)
/=============================================================================================*/
 public void setDisplaySun(boolean state){
     isDisplaySun = state;
 }
...
/*=============================================================================================
/     etDisplayMoon(boolean state)
/=============================================================================================*/
 public void setDisplayMoon(boolean state){
     isDisplayMoon = state;
 }
 public boolean isDisplayMoon(){
     return isDisplayMoon;
 }
 /*=============================================================================================
/     etDisplayMoon(boolean state)
/=============================================================================================*/
 public void setDisplayBrightStars(boolean state){
     isDisplayBrightStars = state;
 }
 public boolean isDisplayBrightStars(){
     return isDisplayBrightStars;
 }
 }