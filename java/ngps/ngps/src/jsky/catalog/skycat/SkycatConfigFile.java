/*
 * $Id: SkycatConfigFile.java,v 1.4 2009/04/21 13:31:17 abrighto Exp $
 */
 ...
 * @version $Revision: 1.4 $ $Date: 2009/04/21 13:31:17 $
 */
public class SkycatConfigFile extends AbstractCatalogDirectory {
    ...
    /**
     * Load the skycat style config file from resources
     *
    ...
    /**
     * This method is called for references to other skycat.cfg files.
     */
     ...
     *
     * @param cat contains the information from the config file.
     */
    public static CatalogDirectory getDirectory(AstroCatalog cat) {
        SkycatConfigFile configFile = getConfigFile();
        configFile.setName(cat.getName()); // XXX allow more configuration
        return configFile;
    }
     ...
     * <p/>
     * Next, the file ~/.jsky3/skycat.cfg is checked. This file is created when
    ...
    public List<Catalog> getNameServers() {
        List<Catalog> l = CatalogFactory.getCatalogsByType(Catalog.NAME_SERVER);
        if (l.size() != 0) {
            return l;
        }
     ...
     * (~/.jsky3/skycat.cfg) to make it permanent. The information is saved in the
    ...
    public void save(String filename) throws IOException {
        ...
        if (!dir.isDirectory()) {
            if (!dir.mkdirs()) {
                DialogUtil.error("Could not create directory: " + dir);
                return;
            }
        }
        ...
        if (!newFile.delete()) { // needed under Windows!
            DialogUtil.error("Could not delete existing file: " + file);
            return;
        }
