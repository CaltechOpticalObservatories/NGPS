...
namespace Filter {
  ...
  class ControllerInfo {
    ...
    public:
      ...
      long load_info( std::string &input ) {
        std::string function = "  (Filter::ControllerInfo::load_info) ";
  ...
  class Interface {
    ...
    public:
      ...
      //
      std::vector<Filter::ControllerInfo> controller_info;

      long parse_command( std::string cmd, std::string &retstring );
      long test();
      static void do_home( Filter::ControllerInfo &info, std::mutex &mlock );
      static void do_move( Filter::ControllerInfo &info, std::mutex &mlock, int distance, float pos );
