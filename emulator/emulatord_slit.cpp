...
void signal_handler( int signo ) {
  std::string function = "  (SlitEmulator::signal_handler) ";
  switch ( signo ) {
    case SIGTERM:
    case SIGINT:
...
int main( int argc, char **argv ) {
  std::string function = "  (SlitEmulator::main) ";
...
*/
  ...
  if ( s.Listen() < 0 ) {                                          // create listening socket
    ...
    std::cerr << get_timestamp() << function << "spawning listening thread on port " << emulator.port << "\n";
  }
...
void block_main( Network::TcpSocket sock ) {
  while(1) {
    int fd = sock.Accept();
    std::cerr << get_timestamp() << "  (SlitEmulator::block_main) Accept returns connection on fd = " << fd << "\n";
...
void doit( Network::TcpSocket sock ) {
  std::string function = "  (SlitEmulator::doit) ";
  long  ret;
  std::stringstream message;
