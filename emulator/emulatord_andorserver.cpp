/**
 * @file    emulatord_andorserver.cpp
...
#include "emulatord_andorserver.h"
#include "daemonize.h"

AndorServerEmulator::Server emulator;
...
void signal_handler( int signo ) {
  std::string function = "  (AndorServerEmulator::signal_handler) ";
  switch ( signo ) {
    case SIGTERM:
    case SIGINT:
...
int main( int argc, char **argv ) {
  std::string function = "  (AndorServerEmulator::main) ";
...
void block_main( Network::TcpSocket sock ) {
  while(1) {
    int fd = sock.Accept();
    std::cerr << get_timestamp() << "  (AndorServerEmulator::block_main) Accept returns connection on fd = " << fd << "\n";
...
void doit( Network::TcpSocket sock ) {
  std::string function = "  (AndorServerEmulator::doit) ";
  long  ret;
  std::stringstream message;
  std::string cmd, args;        // arg string is everything after command
  std::vector<std::string> tokens;
  char delim = '\n';           /// commands sent to me (the TCS) have been terminated with this
  std::string term = "\n";     /// my replies (as the TCS) get terminated with this
  ...
  while (connection_open) {
    ...
    if ( cmd.compare( "acquire" ) == 0 ) {
                    ret = emulator.interface.acquire( args, retstring );
    }
    else
    if ( cmd.compare( ACAMD_CAMERASERVER_COORDS ) == 0 ) {
                    ret = emulator.interface.coords( args );
                    retstring="ACK";
    }
    ...
    if (ret != NOTHING) {
      // append a terminator to the retstring
      //
      retstring.append( "\n" );
