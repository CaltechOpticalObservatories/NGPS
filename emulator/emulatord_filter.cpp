...
void doit( Network::TcpSocket sock ) {
  ...
  while (connection_open) {
    ...
    if ( (ret=sock.Read( sbuf, delim )) <= 0 ) {     // read until newline delimiter
      ...
      if (ret==0) std::cerr << get_timestamp() << function << emulator.subsystem << " timeout reading from socket\n";
    ...
    try {
      ...
      std::cerr << get_timestamp() << function << emulator.subsystem << " received command (" << sock.id << "): " << cmd << " " << args << "\n";
    }
