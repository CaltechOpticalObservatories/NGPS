...
#include "emulatord_calib.h"
#include "daemonize.h"

CalibEmulator::Server emulator;
...
void signal_handler( int signo ) {
  std::string function = "  (CalibEmulator::signal_handler) ";
  switch ( signo ) {
    case SIGTERM:
    case SIGINT:
...
int main( int argc, char **argv ) {
  std::string function = "  (CalibEmulator::main) ";
...
void block_main( Network::TcpSocket sock ) {
  while(1) {
    int fd = sock.Accept();
    std::cerr << get_timestamp() << "  (CalibEmulator::block_main) Accept returns connection on fd = " << fd << "\n";
...
void doit( Network::TcpSocket sock ) {
  std::string function = "  (CalibEmulator::doit) ";
  long  ret;
  std::stringstream message;
