...
//    flexured.common.message.enqueue("exit");   // shutdown the async_main thread if running
      flexured.exit_cleanly();                   // shutdown the daemon
      break;
    case SIGHUP:
      logwrite(function, "caught SIGHUP");
      flexured.configure_flexured();             // TODO can (/should) this be done while running?
      break;
    case SIGPIPE:
...
void thread_main(Network::TcpSocket sock) {
  while (1) {
    flexured.conn_mutex.lock();
    sock.Accept();
    flexured.conn_mutex.unlock();
    doit(sock);                // call function to do the work
    sock.Close();
  }
  return;
...
void async_main(Network::UdpSocket sock) {
  std::string function = "Flexure::async_main";
  int retval;
  ...
  if (retval < 0) {
    ...
    flexured.exit_cleanly();                                // do not continue on error
  }
...
/***
  while (1) {
    std::string message = flexured.common.message.dequeue();  // get the latest message from the queue (blocks)
...
void doit(Network::TcpSocket sock) {
  std::string function = "Flexure::doit";
  char  buf[BUFSIZE+1];
  long  ret;
  std::stringstream message;
  ...
  while (connection_open) {
    ...
    try {
      ...
      sock.id = ++flexured.cmd_num;
      if ( flexured.cmd_num == INT_MAX ) flexured.cmd_num = 0;
...
//                  flexured.common.message.enqueue("exit");   // shutdown the async message thread if running
                    flexured.exit_cleanly();                   // shutdown the daemon
    }
