...
namespace Flexure {
  ...
  long Interface::initialize_class() {
    ...
    if ( this->numdev != 4 ) {
      message.str( "" );
      message << "WARNING: " << this->numdev << " PI motor controller" << ( this->numdev == 1 ? "" : "s" )
              << " defined! (expected 4)";
  ...
  long Interface::compensate( std::string args, std::string &retstring ) {
    ...
    for ( const auto &chan : channels ) {
      try {
        if ( this->motors.find( chan ) == this->motors.end() ) throw std::runtime_error( "unrecognized channel: " + chan );
  ...
  void Interface::offset_tiptilt( const std::string &chan, const std::pair<double, double> &delta, bool is_dryrun ) {
    ...
    // nominal positions
    const auto* axisinfo_x = this->motors.at( chan ).axisinfo( AXIS_X );
    const auto* axisinfo_y = this->motors.at( chan ).axisinfo( AXIS_Y );
    if ( !axisinfo_x || !axisinfo_y ) {
      throw std::runtime_error( "missing axis info for channel " + chan );
    }
    float nominal_x = axisinfo_x->defpos;
    float nominal_y = axisinfo_y->defpos;
    ...
    if ( is_dryrun ) {
      std::ostringstream oss;
      oss << "dry run: would move chan " << chan << " by " << delta.first << ", " << delta.second << " to X=" << newposition_x
          << " Y=" << newposition_y;
    ...
    if ( this->motors.at( chan ).moveto( AXIS_X, newposition_x, retstring ) != NO_ERROR ) {
      throw std::runtime_error( "moving X axis for channel " + chan );
    }

    // move Y-axis
    if ( this->motors.at( chan ).moveto( AXIS_Y, newposition_y, retstring ) != NO_ERROR ) {
  ...
  void Interface::get_status() {
    ...
    for ( auto &mot : this->motors ) {
      bool connected = mot.second.is_connected();
      const auto* axesmap = mot.second.axesmap();
      if ( !axesmap ) continue;
      for ( const auto &axis : *axesmap ) {
        auto chan = mot.first;
        ...
        std::string key;
        if ( connected ) {
          mot.second.get_pos( axis.second.axisnum, addr, position, posname );
        }
        switch ( axis.second.axisnum ) {
          case AXIS_Z:
            key = Key::Flexured::AxisZ + chan;
            break;
          case AXIS_X:
            key = Key::Flexured::AxisX + chan;
            break;
          case AXIS_Y:
            key = Key::Flexured::AxisY + chan;
            break;
          default: key = "error";
            message.str( "" );
            message << "ERROR unknown axis " << axis.second.axisnum;
            logwrite( function, message.str() );
            continue;
        }
  ...
  void Interface::publish_status( bool force ) {
    ...
    try {
      ...
      logwrite( "Flexure::Interface::publish_status", "ERROR publishing message: " + std::string( e.what() ) );
    }
  }
  /***** Flexure::Interface::publish_status **********************************/
      ...
      if ( testname == "shift" ) {
        ...
        if ( testname == "comp" ) {
          ...
          try {
            ...
            message.str( "" );
            message << "delta X=" << delta.first << " Y=" << delta.second;
            logwrite( function, message.str() );
