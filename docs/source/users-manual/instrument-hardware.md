...
```{figure} assets/instrument-hardware-01.png
...
- **Shutter** — The shutter mechanism controls the exposure length and light impinging upon the detectors. A single shutter is used for all detectors, imparting on the detector controller certain synchronization requirements.

- **Dichroic tree** — Optical assembly containing three dichroic beam splitters and a mirror that performs spectral separation and folds the beam horizontally.

- **Collimator(s)** — Forms a pupil on the grating where a mask blocks light originating outside the telescope beam; piezo actuated for flexure compensation in the dispersion direction. Also includes the focus axis for the U channel.

- **Diffraction grating(s)** — Transmissive diffraction gratings disperse the light in the spectral direction.

- **Camera(s)** — Refocuses the dispersed light onto the detector(s). For three channels (G, R, and I) also includes a focus mechanism.
...
- **Acquisition and Guiding (A&G) subsystem** — Determines initial telescope pointing, commands telescope to move so that the desired target is correctly positioned on the slicer/slit, then continuously updates the pointing correction.

- **Slice viewing subsystem** — Provides visual confirmation of the alignment between target and slicer/slit; user may respond by invoking a manual pointing offset.

- **Instrument electronics** — Provide a means to control and monitor the instrument, its performance and health. Housed in a dedicated electronics cabinet attached to the underside of the structure.
...
```{figure} assets/instrument-hardware-02.jpg
:alt: NGPS manual figure
:width: 90%
```

```{figure} assets/instrument-hardware-03.jpg
:alt: NGPS manual figure
:width: 90%
```
